...
if(null == (String)username){
    ...
    //out.println("<h1> <center>Order History</center> </h1>\n" );
    ...
    try {
        ...
        if(null == filter){
            map = MySQLDataStoreUtilities.selectAllOrdersForUser((String)userId);
        }else{
            map = MySQLDataStoreUtilities.selectAllOrdersWithFilterForUser(filter, (String)userId);
        }
    ...
    if (map.isEmpty()){
        out.println("<h3 style=\"color:#cc0000;\">No items in order history.</h3>");
        ...
        for(Map.Entry<String,saxParser.Order> m :map.entrySet()){
            out.println("<ul>");
            out.println("<form action=\"/csj/jsp/customer/AddDeleteUpdateOrder.jsp\">");
            ...
            out.println("<button class=\"btn btn-success btn-sm\" type=submit name=Delete value="+m.getValue().getId()+">Cancel Order</button>");
